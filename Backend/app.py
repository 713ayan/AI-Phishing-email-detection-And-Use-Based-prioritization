...
def ai_predict_internal(subject, body, sender):
    trusted_domains = [
        '@marketing.goindigo.in', '@hdfcbank.net', '@swiggy.in', '@ihcltata.com',
        '@indusind.com', '@amazon.in', '@unstop.news', '@hirist.tech', '@sitpune.edu.in','@email.meetup.com','@email2.microsoft.com','@linkdin.com','@indeed.com','@accounts.google.com'
    ]
    
    try:
        # Check if the sender's domain matches any of the trusted domains
        if any(sender.lower().endswith(domain) for domain in trusted_domains):
            priority, priority_score = calculate_priority(subject, body, sender)
            return {
                'ml_prediction': 'not spam',
                'ml_confidence': 1.0,
                'ai_prediction': 'not spam',
                'ai_explanation': 'Email is from a trusted domain, automatically classified as not spam.',
                'final_prediction': 'not spam',
                'custom_message': 'This email is from a trusted domain and will always be considered as not spam.',
                'priority': priority,
                'priority_score': priority_score,
                'suspicious_links': []  # No phishing check needed for trusted domains
            }

        # Continue with the existing logic for non-trusted senders
...
def get_emails():
    ...
    max_emails = 30  # Adjust the limit as needed
        ...
        if email_message.is_multipart():
            for part in email_message.walk():
                if part.get_content_type() == "text/plain":
                    try:
                        email_info["body"] = clean_text(part.get_payload(decode=True).decode('utf-8', errors='ignore'))
                    except UnicodeDecodeError:
                        email_info["body"] = clean_text(part.get_payload(decode=True).decode('ISO-8859-1', errors='ignore'))
        else:
            try:
                email_info["body"] = clean_text(email_message.get_payload(decode=True).decode('utf-8', errors='ignore'))
            except UnicodeDecodeError:
                email_info["body"] = clean_text(email_message.get_payload(decode=True).decode('ISO-8859-1', errors='ignore'))
